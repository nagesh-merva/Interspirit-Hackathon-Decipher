...
function EngagementMonitoring() {
  // Sample data for engagement rate
  ...
  // Sample data for emotional tone
  ...
  const maxValue = Math.max(
    ...emotionalToneData.map(item => Math.max(item.twitter, item.instagram))
  )
                ...
                <div className="flex-1 flex items-center space-x-2">
                  <div className="flex-1 h-6 bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(item.twitter / maxValue) * 100}%` }}
                    >
                      <span className="text-xs text-white">{item.twitter}%</span>
                    </div>
                  </div>
                  <div className="flex-1 h-6 bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden">
                    <div
                      className="h-full bg-pink-400 flex items-center justify-end pr-1"
                      style={{ width: `${(item.instagram / maxValue) * 100}%` }}
                    >
                      <span className="text-xs text-white">{item.instagram}%</span>
                    </div>
                  </div>
                </div>
              </div>
